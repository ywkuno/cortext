...
Then read the generated slice Markdown. If a specific node ID is relevant, run `codeprism references NODE_ID` and `codeprism get NODE_ID`. If a whole file might be needed, run `codeprism read PATH --mode signatures` or `codeprism read PATH --mode diff` before opening the full file. Answer with file paths and line references where possible.
